...
TSaslTransport::TSaslTransport(std::shared_ptr<sasl::TSasl> saslClient, std::shared_ptr<TTransport> transport)
        : TVirtualTransport(transport->getConfiguration()),
          transport_(std::move(transport)),
          memBuf_(new TMemoryBuffer(transport_->getConfiguration())),
          sasl_(std::move(saslClient)),
          shouldWrap_(false),
          isClient_(true) {}
...
std::string TSaslTransport::getUsername() {
    return sasl_->getUsername();
}

std::string TSaslTransport::getMechanismName() {
...
void TSaslTransport::doSaslNegotiation() {
    ...
    try {
        ...
        while (!sasl_->isComplete()) {
            ...
            uint8_t* challenge = sasl_->evaluateChallengeOrResponse(message, resLength, &challengeLength);
            sendSaslMessage(sasl_->isComplete() ? TSASL_COMPLETE : TSASL_OK, challenge, challengeLength);
        }
...
uint32_t TSaslTransport::read(uint8_t* buf, uint32_t len) {
    ...
    auto* tmpBuf = new uint8_t[dataLength];
...
void TSaslTransport::sendSaslMessage(const NegotiationStatus status, const uint8_t* payload, const uint32_t length,
                                     bool flush) {
    ...
    messageHeader[0] = static_cast<uint8_t>(status);
...
uint8_t* TSaslTransport::receiveSaslMessage(NegotiationStatus* status, uint32_t* length) {
    ...
    // get payload status
    *status = static_cast<NegotiationStatus>(messageHeader[0]);
